...
def compute_eas_j2_contributions_jax(
    ...
    def _alpha_step(alpha_k, _):
        ...
        for k in range(4):
            Ft = Fc_all[k] + jnp.einsum('j,jab->ab', alpha_k, Fenh_all[k])
            S_v, C_v, _ = tangent_voigt_jax(
                Ft[:2, :2], state_elem[k], lam, mu, sigma_y0, H,
            )
            G = _compute_G(Ft, Fenh_all[k])
            St = jnp.array([[S_v[0], S_v[2]], [S_v[2], S_v[1]]])

            f_a = f_a + G.T @ S_v * w_all[k]
            K_aa = K_aa + (G.T @ C_v @ G + _compute_Kgeo_aa(Fenh_all[k], St)) * w_all[k]

        K_reg = K_aa + 1e-12 * (jnp.trace(K_aa) / 4.0 + 1e-30) * jnp.eye(4)
    ...
    for k in range(4):
        Ft = Fc_all[k] + jnp.einsum('j,jab->ab', alpha_final, Fenh_all[k])
        ...
        St = jnp.array([[S_v[0], S_v[2]], [S_v[2], S_v[1]]])
        BL = _BL_columns(Ft, gX_all[k], gY_all[k])
        G = _compute_G(Ft, Fenh_all[k])
