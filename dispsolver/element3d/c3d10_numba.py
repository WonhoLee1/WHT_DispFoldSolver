...
c3d10_numba.py
==============
Numba Kernel for Standard 3D 10-Node Quadratic Tetrahedron (C3D10) Elements.

Note: Abaqus C3D10M modifies shape functions for contact consistency (strictly positive
nodal forces) and enforces constant volumetric strain with hourglass stabilization.
This kernel is an honest standard 4-point quadratic tetrahedron (C3D10).
"""
...
if HAS_NUMBA:
    ...
    @njit(fastmath=True)
    def compute_c3d10_element_numba(coords: np.ndarray, u_elem: np.ndarray, C_mat: np.ndarray):
    ...
    def assemble_mesh_c3d10_numba(
        node_coords: np.ndarray,
        elem_conn: np.ndarray,
        u_global: np.ndarray,
        C_mat: np.ndarray
    ):
        """Parallel OpenMP mesh assembly kernel for C3D10 Tet10 elements."""
        ...
        for e in prange(n_elems):
            ...
            Ke, fe = compute_c3d10_element_numba(coords_e, u_e, C_mat)
            K_elems[e] = Ke
            f_elems[e] = fe
    ...
    # Legacy aliases
    compute_c3d10m_element_numba = compute_c3d10_element_numba
    assemble_mesh_c3d10m_numba = assemble_mesh_c3d10_numba
