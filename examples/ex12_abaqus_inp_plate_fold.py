...
def run_abaqus_inp_folding(inp_path: str = None, before_png_name: str = "ex12_before_folding_shape.png",
    ...
    NOTE: `config` here only controls solver tuning/element type (see
    `run_folding_from_result`) -- it does NOT change the geometry/
    materials baked into `inp_path` already. To change those, re-run
    `gen_ex12_inp.generate(config)` first (see README_folding_model.md).
    """
    print("=" * 100)
    ...
    return run_folding_from_result(result, before_png_name, after_png_name,
                                   ...
                                   result_name=result_name, config=config)
